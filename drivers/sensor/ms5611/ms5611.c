			       ...
			       enum sensor_channel channel)
{
	...
	err = ms5611_get_measurement(config,
				     &adc_pressure,
...
static int ms5611_init(const struct device *dev)
{
	...
	err = ms5611_attr_set(dev, SENSOR_CHAN_PRESS, SENSOR_ATTR_OVERSAMPLING,
			      &val);
	if (err < 0) {
		return err;
	}
	...
	err = ms5611_attr_set(dev, SENSOR_CHAN_AMBIENT_TEMP,
	...
	err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_OFF_T1,
			       &data->off_t1);
	if (err < 0) {
		return err;
	}
	...
	err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_SENSE_T1,
			       &data->sens_t1);
	if (err < 0) {
		return err;
	}
	...
	err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_TEMPSENS,
			       &data->tempsens);
	if (err < 0) {
		return err;
	}
...
#define MS5611_SPI_OPERATION (SPI_OP_MODE_MASTER | SPI_WORD_SET(8) |	\
