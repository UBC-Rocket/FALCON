...
LOG_MODULE_REGISTER(ms5611);

static void ms5611_compensate(struct ms5611_data *data, const int32_t adc_temperature,
...
static int ms5611_read_prom(const struct ms5611_config *config, uint8_t cmd, uint16_t *val)
{
    int err;
...
static int ms5611_get_measurement(const struct ms5611_config *config, uint32_t *val, uint8_t cmd,
                                  uint8_t delay)
{
    int err;
...
static int ms5611_sample_fetch(const struct device *dev, enum sensor_channel channel)
{
    const struct ms5611_config *config = dev->config;
    ...
    err = ms5611_get_measurement(config, &adc_pressure, data->pressure_conv_cmd,
                                 data->pressure_conv_delay);
    if (err < 0) {
        return err;
    }

    err = ms5611_get_measurement(config, &adc_temperature, data->temperature_conv_cmd,
                                 data->temperature_conv_delay);
    if (err < 0) {
        return err;
    }

    ms5611_compensate(data, adc_temperature, adc_pressure);
    return 0;
}
                           ...
                           enum sensor_attribute attr, const struct sensor_value *val)
{
    ...
    switch (val->val1) {
    case 4096:
        p_conv_cmd = MS5611_CMD_CONV_P_4096;
        t_conv_cmd = MS5611_CMD_CONV_T_4096;
        conv_delay = 9U;
        break;
    case 2048:
        p_conv_cmd = MS5611_CMD_CONV_P_2048;
        t_conv_cmd = MS5611_CMD_CONV_T_2048;
        conv_delay = 5U;
        break;
    case 1024:
        p_conv_cmd = MS5611_CMD_CONV_P_1024;
        t_conv_cmd = MS5611_CMD_CONV_T_1024;
        conv_delay = 3U;
        break;
    case 512:
        p_conv_cmd = MS5611_CMD_CONV_P_512;
        t_conv_cmd = MS5611_CMD_CONV_T_512;
        conv_delay = 2U;
        break;
    case 256:
        p_conv_cmd = MS5611_CMD_CONV_P_256;
        t_conv_cmd = MS5611_CMD_CONV_T_256;
        conv_delay = 1U;
        break;
    default:
...
{
    const struct ms5611_config *const config = dev->config;
    struct ms5611_data *data = dev->data;
    struct sensor_value val;
    int err;
    ...
    val.val1 = MS5611_PRES_OVER_DEFAULT;
    err = ms5611_attr_set(dev, SENSOR_CHAN_PRESS, SENSOR_ATTR_OVERSAMPLING, &val);
    if (err < 0) {
        return err;
    }

    val.val1 = MS5611_TEMP_OVER_DEFAULT;
    err = ms5611_attr_set(dev, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_OVERSAMPLING, &val);
    if (err < 0) {
        return err;
    }
    ...
    err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_OFF_T1, &data->off_t1);
    if (err < 0) {
        return err;
    }
    ...
    err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_SENSE_T1, &data->sens_t1);
    if (err < 0) {
        return err;
    }
    ...
    err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_T_REF, &data->t_ref);
    if (err < 0) {
        return err;
    }
    ...
    err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_TCO, &data->tco);
    if (err < 0) {
        return err;
    }
    ...
    err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_TCS, &data->tcs);
    if (err < 0) {
        return err;
    }
    ...
    err = ms5611_read_prom(config, MS5611_CMD_CONV_READ_TEMPSENS, &data->tempsens);
    if (err < 0) {
        return err;
    }
 ...
 */
#define MS5611_DEFINE(inst)                                                                       \
    static struct ms5611_data ms5611_data_##inst;                                                 \
    static const struct ms5611_config ms5611_config_##inst = COND_CODE_1(                         \
        DT_INST_ON_BUS(inst, spi), (MS5611_CONFIG_SPI(inst)), (MS5611_CONFIG_I2C(inst)));         \
    SENSOR_DEVICE_DT_INST_DEFINE(inst, ms5611_init, NULL, &ms5611_data_##inst,                    \
                                 &ms5611_config_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, \
                                 &ms5611_api_funcs);

/* Create the struct device for every status "okay" node in the devicetree. */
DT_INST_FOREACH_STATUS_OKAY(MS5611_DEFINE)
