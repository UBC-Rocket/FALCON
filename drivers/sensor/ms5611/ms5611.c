...
static void ms5611_compensate(struct ms5611_data *data,
  ...
  temp_sq =
      (int64_t)(data->temperature - 2000) * (int64_t)(data->temperature - 2000);
  ...
  data->temperature -= Ti;
  data->pressure =
      (SENS * (int64_t)adc_pressure / (1ll << 21) - OFF) / (1ll << 15);
}

static int ms5611_read_prom(const struct ms5611_config *config, uint8_t cmd,
                            uint16_t *val) {
  int err;
...
static int ms5611_get_measurement(const struct ms5611_config *config,
                                  uint32_t *val, uint8_t cmd, uint8_t delay) {
  int err;
...
static int ms5611_sample_fetch(const struct device *dev,
  ...
  err = ms5611_get_measurement(config, &adc_temperature,
...
static int ms5611_init(const struct device *dev) {
  ...
  err = ms5611_attr_set(dev, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_OVERSAMPLING,
                        &val);
  if (err < 0) {
    return err;
  }
  ...
  err =
      ms5611_read_prom(config, MS5611_CMD_CONV_READ_TEMPSENS, &data->tempsens);
  if (err < 0) {
    return err;
  }
...
#define MS5611_DEFINE(inst)                                                    \
  ...
  static const struct ms5611_config ms5611_config_##inst =                     \
      COND_CODE_1(DT_INST_ON_BUS(inst, spi), (MS5611_CONFIG_SPI(inst)),        \
                  (MS5611_CONFIG_I2C(inst)));                                  \
  SENSOR_DEVICE_DT_INST_DEFINE(                                                \
      inst, ms5611_init, NULL, &ms5611_data_##inst, &ms5611_config_##inst,     \
      POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &ms5611_api_funcs);
