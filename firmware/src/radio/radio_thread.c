...
static void radio_thread_fn(void *p1, void *p2, void *p3)
{
    ...
    while (1) {
        ...
        if (!status) {
            LOG_ERR("Failed to encode TelemetryPacket: %s", PB_GET_ERROR(&stream));
            counter++;
        ...
        if (ret == 0) {
            ...
            if (ret < 0) {
                ...
                LOG_INF("Sent telemetry: counter=%u, alt=%.1f, vel=%.1f, "
                        ...
                        counter, (double)message.kf_altitude, (double)message.kf_velocity,
                        (int)message.state, message_length, dst_buf->len);
            }
        } else {
...
{
    k_thread_create(&radio_thread, radio_stack, K_THREAD_STACK_SIZEOF(radio_stack), radio_thread_fn,
                    NULL, NULL, NULL, RADIO_THREAD_PRIORITY, 0, K_NO_WAIT);
}
