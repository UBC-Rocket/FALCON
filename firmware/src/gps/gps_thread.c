...
static void gps_thread_fn(void *p1, void *p2, void *p3)
{
    ...
    while (1) {
        ...
        LOG_INF("GPS: lat=%.6f, lon=%.6f, alt=%.1f m, "
                ...
                (double)gps.latitude, (double)gps.longitude, (double)gps.altitude, gps.sats_in_use,
                gps.fix, (double)gps.speed);
...
{
    k_thread_create(&gps_thread, gps_stack, K_THREAD_STACK_SIZEOF(gps_stack), gps_thread_fn, NULL,
                    NULL, NULL, GPS_THREAD_PRIORITY, 0, K_NO_WAIT);
}
