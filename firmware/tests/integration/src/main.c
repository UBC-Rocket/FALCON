...
static void inject_and_wait(float altitude, float velocity, int duration_ms)
{
    ...
    while (k_uptime_get() < end_time) {
        ...
        if (!logged_once) {
            LOG_INF("Continuously injecting: alt=%.2f m, vel=%.2f m/s for %d ms", altitude,
...
ZTEST(integration, test_full_flight_sequence)
{
    ...
    // Ascent
    LOG_INF("\n=== PHASE 2: ASCENT ===");
    LOG_INF("Injecting altitude > %.2f m and velocity > %.2f m/s", ASCENT_ALTITUDE_THRESHOLD_M,
            ASCENT_VELOCITY_THRESHOLD_MPS);
    inject_test_data(ASCENT_ALTITUDE_THRESHOLD_M + 1.0, ASCENT_VELOCITY_THRESHOLD_MPS + 1.0, true);
    k_sleep(K_SECONDS(5));
