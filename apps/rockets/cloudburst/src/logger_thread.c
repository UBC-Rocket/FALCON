...
static int create_new_log_file(void) {
  ...
  snprintf(log_file_name, sizeof(log_file_name), MOUNT_POINT "/log_%d.csv",
           file_count);
  ...
  ret =
      fs_open(&log_file, log_file_name, FS_O_CREATE | FS_O_APPEND | FS_O_WRITE);
  if (ret < 0) {
...
static int format_log_entry(const struct log_frame *frame, char *buffer,
                            size_t buffer_size) {
  return snprintf(
      buffer, buffer_size,
      ...
      (double)frame->imu.accel[0], (double)frame->imu.accel[1],
      (double)frame->imu.accel[2], (double)frame->imu.gyro[0],
      (double)frame->imu.gyro[1], (double)frame->imu.gyro[2],
      ...
      (double)frame->baro.velocity, (double)frame->baro.vel_variance);
}
...
static void write_log_frame_to_file(const struct log_frame *frame) {
  ...
  if (len >= sizeof(log_entry)) {
    ...
    len = sizeof(log_entry) -
          1; // Write only up to the buffer size (excluding null terminator)
...
void start_logger_thread() {
  k_thread_create(&logger_thread, logger_stack,
                  K_THREAD_STACK_SIZEOF(logger_stack), logger_thread_fn, NULL,
                  NULL, NULL, LOGGER_THREAD_PRIORITY, 0, K_NO_WAIT);
}
